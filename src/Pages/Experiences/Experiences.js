import React from 'react';
import { Card, Col, Container, Row } from 'react-bootstrap';

const Experiences = () => {
    return (
        <Container className="mt-5">
            <h2 className="text-danger mb-5">Experiences</h2>

            <Row xs={1} md={4} className="g-4">
                <Col>
                    <Card className="border-0 shadow">
                        <Card.Img className="w-50 p-3 mx-auto" variant="top" src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/HTML5_logo_and_wordmark.svg/512px-HTML5_logo_and_wordmark.svg.png" height="200px" />
                        <Card.Body>
                            <Card.Title>HTML 5</Card.Title>
                        </Card.Body>
                    </Card>
                </Col>
                <Col>
                    <Card className="border-0 shadow">
                        <Card.Img className="w-50 p-3 mx-auto" variant="top" src="https://www.seekpng.com/png/full/222-2229373_open-css-3.png" height="200px" />
                        <Card.Body>
                            <Card.Title>CSS 3</Card.Title>
                        </Card.Body>
                    </Card>
                </Col>
                <Col>
                    <Card className="border-0 shadow">
                        <Card.Img className="w-50 p-3 mx-auto" variant="top" src="https://w1.pngwing.com/pngs/951/574/png-transparent-react-logo-javascript-redux-vuejs-angular-angularjs-expressjs-front-and-back-ends-thumbnail.png" height="200px" />
                        <Card.Body>
                            <Card.Title>JavaScript</Card.Title>
                        </Card.Body>
                    </Card>
                </Col>
                <Col>
                    <Card className="border-0 shadow">
                        <Card.Img className="w-50 p-3 mx-auto" variant="top" src="https://cdn.dribbble.com/users/528264/screenshots/3140440/firebase_logo.png" height="200px" />
                        <Card.Body>
                            <Card.Title>Firebase</Card.Title>
                        </Card.Body>
                    </Card>
                </Col>
                <Col>
                    <Card className="border-0 shadow">
                        <Card.Img className="w-100 p-3 mx-auto" variant="top" src="data:image/png;base64," height="200px" />
                        <Card.Body>
                            <Card.Title>ReactJS</Card.Title>
                        </Card.Body>
                    </Card>
                </Col>
                <Col>
                    <Card className="border-0 shadow">
                        <Card.Img className="w-100 p-3 mx-auto" variant="top" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTfZxVauwvK9ORMtJaQf1oRvmaZhAHhx3feXyaleyiYd3Q3BnFrtqAvkN9K2F069mfwEFQ&usqp=CAU" height="200px" />
                        <Card.Body>
                            <Card.Title>NodeJS</Card.Title>
                        </Card.Body>
                    </Card>
                </Col>
                <Col>
                    <Card className="border-0 shadow">
                        <Card.Img className="w-100 p-3 mx-auto" variant="top" src="https://gocode.colorado.gov/wp-content/uploads/2020/11/MongoDB-logo.gif" height="200px" />
                        <Card.Body>
                            <Card.Title>MongoDb</Card.Title>
                        </Card.Body>
                    </Card>
                </Col>
                <Col>
                    <Card className="border-0 shadow">
                        <Card.Img className="w-100 p-3 mx-auto" variant="top" src="https://1.bp.blogspot.com/-jkSmywQ57sA/Wer3KKSqgaI/AAAAAAAACc4/07TexMsBBI4v7WlVKo76YvxM3TvrMxIdwCLcBGAs/s640/express.js.png" height="200px" />
                        <Card.Body>
                            <Card.Title>ExpressJs</Card.Title>
                        </Card.Body>
                    </Card>
                </Col>
            </Row>
        </Container>
    );
};

export default Experiences;